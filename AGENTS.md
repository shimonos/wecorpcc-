...
- Rename everything from the template. Do not leave `com.example`, `WeCorpPlugin`, `WeCorpConfig`, or `example` as the config group. Rename the package path, class names, config group, `build.gradle` group, `settings.gradle` project name, and `runelite-plugin.properties`.
